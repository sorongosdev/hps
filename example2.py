...
def main():
    ...
    for chunk in buffer_chunks[0: 60]:
        ...
        # 주파수 배열에서 1000 Hz 이하의 값에 해당하는 인덱스를 찾습니다.
        max_index = np.where(fft_freq <= 1000)[0][-1] + 1

        fft_freq_interval = fft_freq[:max_index]
        fft_res_interval = fft_res[:max_index]

        magnitude = 2.0/N * np.abs(fft_res_interval)

        y_freqs = np.array([freq[0] for freq in all_freqs])

        magnitude_matrix = np.tile(magnitude, (len(y_freqs), 1))

        plt.figure(figsize=(10, 6))
        plt.pcolormesh(fft_freq_interval, y_freqs, magnitude_matrix, shading='auto')
        plt.colorbar(label='Magnitude')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Detected Notes Frequency (Hz)')
        plt.title('Spectrogram-like Representation Using FFT Results')
        # x축의 범위를 설정합니다.
        plt.xlim(0, 1000)
        plt.show()
